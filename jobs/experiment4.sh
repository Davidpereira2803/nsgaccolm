...
#SBATCH --job-name=colmexperiment4
...
python pipeline.py 51 \
  --skip-nsga \
  --model-path colm_model_perm_pipeline4.pth \
  --nsga-run-id nsga_pipeline \
  --db-path solutions.db \
  --top-k 1000 \
  --infer-temperature 0.1 \
  --colm-run-id colm_t01_k1000
